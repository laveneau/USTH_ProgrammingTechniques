"""Any `LinearSolver` must solve SPD systems and refuse non-SPD ones.
...
from optlab.interfaces import LinearSolver
...
def test_solves_an_spd_system(name: str, solver: LinearSolver) -> None:
...
@pytest.mark.day4
@pytest.mark.parametrize("name,solver", ALL_SOLVERS, ids=[n for n, _ in ALL_SOLVERS])
def test_matches_the_numpy_oracle(name: str, solver: LinearSolver, rng) -> None:
...
@pytest.mark.day4
@pytest.mark.parametrize("name,solver", ALL_SOLVERS, ids=[n for n, _ in ALL_SOLVERS])
def test_rejects_a_non_positive_definite_matrix(name: str, solver: LinearSolver) -> None:
